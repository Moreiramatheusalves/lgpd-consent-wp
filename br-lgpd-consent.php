...
if (is_admin()) {
    $puc_path = BRLGPD_DIR . 'vendor/plugin-update-checker/plugin-update-checker.php';

    if (file_exists($puc_path)) {

        $alreadyLoaded =
            class_exists('\YahnisElsts\PluginUpdateChecker\v5\PucFactory', false) ||
            class_exists('\YahnisElsts\PluginUpdateChecker\v4\PucFactory', false) ||
            class_exists('Puc_v4_Factory', false) ||
            class_exists('Puc_v5_Factory', false);
        ...
        } elseif (class_exists('\YahnisElsts\PluginUpdateChecker\v4\PucFactory')) {
            $updateChecker = \YahnisElsts\PluginUpdateChecker\v4\PucFactory::buildUpdateChecker(
                'https://github.com/Moreiramatheusalves/lgpd-consent-wp/',
                __FILE__,
                'lgpd-consent-wp'
            );
        ...
        if (!empty($updateChecker) && is_object($updateChecker)) {
